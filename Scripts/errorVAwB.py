...
beta = Q*Q/(2.0*m*1*1.0*B0)
gamma = math.sqrt(beta*alpha)
vperp = math.sqrt(Q*Q*Bs/(1.0*1.0*m*alpha))
x0 = -1.0
dt = 0.005
...
for line in r:
	...
	xa.append((v0/gamma)*math.sin(gamma*t[it])+((B0+Bs)/alpha)*math.cos(gamma*t[it])+x0-(B0+Bs)/alpha)
	va.append(v0*math.cos(gamma*(t[it]+0.5*dt))-((B0+Bs)*gamma/alpha)*math.sin(gamma*(t[it]+0.5*dt)))
